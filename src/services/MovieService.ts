...
class MovieService {
  ...
  async getRecommendations(movieId: number, page: number = 1): Promise<SearchResponse> {
    try {
      const response = await fetch(`/api/movies/${movieId}/recommendations?page=${page}`)
      if (!response.ok) {
  ...
  async getTrendingMovies(page: number = 1): Promise<SearchResponse> {
    try {
      const response = await fetch(`/api/movies/trending?type=movies&page=${page}`)
      if (!response.ok) {

...
export const TVShowDetails: React.FC<TVShowDetailsProps> = ({ tvShowId }) => {
  ...
  useEffect(() => {
    const fetchShowData = async () => {
      try {
        setLoading(true);
        const [showData, recommendationsData, castData, videosData] =
          await Promise.all([
        ...
        const similarShows = filterSimilarTVShows(
  ...
  const filterSimilarTVShows = (shows: TVShow[], currentName: string) => {
    ...
    let similarShows = shows.filter((show) => {
      ...
      const commonWords = currentWords.filter((word) =>
        showWords.some(
          (showWord) => showWord.includes(word) || word.includes(showWord),
        ),
      );
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center space-x-4 mb-4">
                  <span
                    className={cn(
                      ...
                      getQualityBadgeColor(show.vote_average),
                    )}
                  >
                ...
                <div className="space-y-3 text-sm">
                  <div className="flex items-center space-x-2">
                    ...
                    <span>
                      {show.origin_country?.join(", ") || "United States"}
                    </span>
                  </div>
                  <div className="flex items-center space-x-2">

      ...
      try {
        ...
        if (isSearchMode && searchQuery) {
          ...
          switch (category) {
            ...
            case "action":
              results = await fetch(
                `/api/movies/genre/28?page=${pageNum}`,
              ).then((res) => res.json());
              break;
            case "comedy":
              results = await fetch(
                `/api/movies/genre/35?page=${pageNum}`,
              ).then((res) => res.json());
              break;
            case "drama":
              results = await fetch(
                `/api/movies/genre/18?page=${pageNum}`,
              ).then((res) => res.json());
              break;
            case "horror":
              results = await fetch(
                `/api/movies/genre/27?page=${pageNum}`,
              ).then((res) => res.json());
              break;
            case "romance":
              results = await fetch(
                `/api/movies/genre/10749?page=${pageNum}`,
              ).then((res) => res.json());
              break;
            case "sci-fi":
              results = await fetch(
                `/api/movies/genre/878?page=${pageNum}`,
              ).then((res) => res.json());
              break;
            default:
              results = await movieService.getPopularMovies(pageNum);
    ...
    return (
      <div className="flex h-64 items-center justify-center">
        <div className="flex flex-col items-center gap-2 text-center">
          ...
          <Button onClick={() => fetchMovies(1)} className="mt-2">
            Try Again
          </Button>
        </div>
      </div>
